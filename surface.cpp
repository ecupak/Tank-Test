...
void Surface::AlphaBar(int x1, int y1, int x2, int y2, Pixel overColor, float overColorOpacity)
{
	Pixel* baseColor = x1 + y1 * m_Pitch + m_Buffer;
	for (int y = y1; y <= y2; y++)
	{
		for (int x = 0; x <= (x2 - x1); x++)
		{
			baseColor[x] = MixAlpha(overColor, overColorOpacity, baseColor[x]);
		}
		baseColor += m_Pitch;
	}
}
...
void Sprite::Draw( Surface* a_Target, int a_X, int a_Y, bool isAlpha, float opacity )
{
...
void Sprite::DrawScaled(int a_X, int a_Y, int a_Width, int a_Height, Surface* a_Target)
{
	...
	// Exit if none of the result will be on screen.
	if ((a_X + a_Width < 0) || (a_X >= a_Target->GetWidth())) return;
	if ((a_Y + a_Height < 0) || (a_Y >= a_Target->GetHeight())) return;

	// Get bounds within a_Target.
	int x1{ Max(0, a_X) };
	int y1{ Max(0, a_Y) };
	int x2{ Min(a_Target->GetWidth(), a_X + a_Width) };
	int y2{ Min(a_Target->GetHeight(), a_Y + a_Height) };

	// If negative x1/y1, find offset for source buffer.
	//int x_offset{ a_X < 0 ? static_cast<int>(floor(-1.0f * a_X / a_Width)) : 0 };
	//int y_offset{ a_Y < 0 ? static_cast<int>(floor(-1.0f * a_Y / a_Height)) : 0 };
	int x_offset{ a_X < 0 ? -a_X : 0 };
	int y_offset{ a_Y < 0 ? -a_Y : 0 };

	// Get starting buffer position.
	Pixel* destination{ a_Target->GetBuffer() + x1 + (y1 * a_Target->GetPitch()) };
	Pixel* source{ GetBuffer() + (m_CurrentFrame * m_Width) };
	...
	for (int y{ 0 }; y < height; y++)
	{
		...
		{
			// Get source pixel of fractional step (double dip+).
			int u = (int)((float)(x + x_offset) * ((float)m_Width / (float)a_Width));
			int v = (int)((float)(y + y_offset) * ((float)m_Height / (float)a_Height));

			// If not a transparent color, copy to destination.
			Pixel color{ *(source + u + (v * m_Pitch)) };
			if (color & 0xffffff)
			{
				destination[x] = color;
			}
		}
		destination += a_Target->GetPitch();
	}
}
