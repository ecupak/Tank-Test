...
inline Pixel MixAlpha(Pixel over_color, float over_color_opacity, Pixel under_color)
{
	...
	unsigned int oB = (over_color & BlueMask);

	// Extract under color values.
	unsigned int uR = (under_color & RedMask) >> 16;
...
public:
	...
	void Bar(int x1, int y1, int x2, int y2, Pixel c, bool hasOpacity = false, float opacity = 1.0f);
	void AlphaBar(int x1, int y1, int x2, int y2, Pixel c, float opacity);
...
public:
	...
	~Sprite();
	// Methods
	void Draw(Surface* a_Target, int a_X, int a_Y, bool isAlpha = false, float opacity = 1.0f);
